...
def read_file(file_path):
    ...
    elif file_extension in ['.xls', '.xlsx', '.xlsm', '.xlsb', '.ods']:
...
try:
    data = read_file("FilePath.csv")

    # Ensure column "a" is present and process it
    if 'a' in data.columns:
        data['a'] = data['a'].astype(str)  # Convert to string for case insensitive comparison
        result = data[data['a'].str.contains('sdas', case=False)]

        # Save the resulting DataFrame to a new CSV file
        result.to_csv("doData_Output_File.csv", index=False)
    else:
        print("Column 'a' not found in the data.")
